...
export default function Chat({ error, messages, setMessages, input, handleInputChange, handleSubmit, verseData, similarVerses }: Props) {
    ...
    return (
        <CollapsibleSection headerText="Chat">
            ...
            {verseData?.target?.content && <div>
                <p>{`Expected target output: ${verseData?.target.content}`}</p>
            </div>}
            <div className="flex flex-col md:flex-row">
                <div className="rounded-md border md:w-3/4 h-[600px]">
                    <ScrollArea className="h-[600px] p-4">
                        {messages.map((message, idx) => <ChatMessage key={message.id} messageIndex={idx} messages={messages} availableTokens={availableTokens} />)}
                    </ScrollArea>
